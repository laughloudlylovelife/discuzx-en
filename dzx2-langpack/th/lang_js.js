/*
	[Discuz!] (C)2001-2009 Comsenz Inc.
	This is NOT a freeware, use is subject to license terms

	Javascript variables for Thai Language

	$Id: static/js/lang_js.js by Valery Votintsev, vot at sources.ru

*/

//--------------------------------
//static/js/common.js

var colortexts = {
	'Black'			: 'สีดำ',//'黑色',
	'Sienna'		: 'สีน้ำตาล',//'赭色',
	'DarkOliveGreen'	: 'สีเขียวมะกอก',//'暗橄榄绿色',
	'DarkGreen'		: 'สีเขียวเข้ม',//'暗绿色',
	'DarkSlateBlue'		: 'สีน้ำเงินนวล',//'暗灰蓝色',
	'Navy'			: 'สีกรมท่า',//'海军色',
	'Indigo'		: 'สีคราม',//'靛青色',
	'DarkSlateGray'		: 'สีเทานวล',//'墨绿色',
	'DarkRed'		: 'สีแดงเข้ม',//'暗红色',
	'DarkOrange'		: 'สีส้มเข้ม',//'暗桔黄色',
	'Olive'			: 'สีมะกอก',//'橄榄色',
	'Green'			: 'สีเขียว',//'绿色',
	'Teal'			: 'สีนกเป็ดน้ำ',//'水鸭色',
	'Blue'			: 'สีน้ำเงิน',//'蓝色',
	'SlateGray'		: 'สีเทา',//'灰石色',
	'DimGray'		: 'สีเทาเข้ม',//'暗灰色',
	'Red'			: 'สีแดง',//'红色',
	'SandyBrown'		: 'สีน้ำตาลทราย',//'沙褐色',
	'YellowGreen'		: 'สีเหลืองเขียว',//'黄绿色',
	'SeaGreen'		: 'สีเขียวน้ำทะเล',//'海绿色',
	'MediumTurquoise'	: 'สีเขียวฟ้า',//'间绿宝石',
	'RoyalBlue'		: 'สีน้ำเงินเข้ม',//'皇家蓝',
	'Purple'		: 'สีม่วง',//'紫色',
	'Gray'			: 'สีเทา',//'灰色',
	'Magenta'		: 'สีม่วงแดง',//'红紫色',
	'Orange'		: 'สีส้ม',//'橙色',
	'Yellow'		: 'สีเหลือง',//'黄色',
	'Lime'			: 'สีมะนาว',//'酸橙色',
	'Cyan'			: 'สีน้ำเงินเขียว',//'青色',
	'DeepSkyBlue'		: 'สีน้ำเงินเข้ม',//'深天蓝色',
	'DarkOrchid'		: 'สีม่วงเข้ม',//'暗紫色',
	'Silver'		: 'สีเงิน',//'银色',
	'Pink'			: 'สีชมพู',//'粉色',
	'Wheat'			: 'สีเหลืองอ่อน',//'浅黄色',
	'LemonChiffon'		: 'สีครีม',//'柠檬绸色',
	'PaleGreen'		: 'สีเขียวซีด',//'苍绿色',
	'PaleTurquoise'		: 'สีฟ้าซีด',//'苍宝石绿',
	'LightBlue'		: 'สีฟ้าสดใส',//'亮蓝色',
	'Plum'			: 'สีม่วงอ่อน',//'洋李色',
	'White'			: 'สีขาว' //'白色'
};

/*
',',	//'，',
'.',	//'。',
':',	//'点',
'!',	//'！'
'&laquo;',//'《',
'&raquo;',//'》',
*/


//--------------------------------

var lng = {

//---------------------------
//static/image/editor/editor_base.js
// USED in /source/module/home/home_editor.php
// MOVE TO:
//static/js/editor_base.js
	'restore_last_saved'	: 'แน่ใจหรือว่าคุณต้องการกู้คืนค่าข้อมูลที่บันทึกไว้ล่าสุด?',//'您确定要恢复上次保存?',
	'cut_manually'		: 'เบราว์เซอร์ของคุณได้ถูกตั้งค่าการรักษาความปลอดภัย ไม่อนุญาตให้ดำเนินการตัดข้อความ กรุณาใช้คีย์ลัด (Ctrl+X)',//'您的浏览器安全设置不允许编辑器自动执行剪切操作,请使用键盘快捷键(Ctrl+X)来完成',
	'copy_manually'		: 'เบราว์เซอร์ของคุณได้ถูกตั้งค่าการรักษาความปลอดภัย ไม่อนุญาตให้ดำเนินการคัดลอกโดยอัตโนมัติ กรุณาใช้คีย์ลัด (Ctrl+C)',//'您的浏览器安全设置不允许编辑器自动执行拷贝操作,请使用键盘快捷键(Ctrl+C)来完成',
	'paste_manually'	: 'เบราว์เซอร์ของคุณได้ถูกตั้งค่าการรักษาความปลอดภัย ไม่อนุญาตให้ดำเนินการวางข้อมูลที่คัดลอกไว้ กรุณาใช้คีย์ลัด (Ctrl+V)',//'您的浏览器安全设置不允许编辑器自动执行粘贴操作,请使用键盘快捷键(Ctrl+V)来完成',
	'graffiti_no_init'	: 'ไม่พบข้อมูลในการวาดรูป',//'找不到涂鸦板初始化数据',
	'ie5_only'		: 'สนับสนุนเฉพาะ IE 5.01 หรือสูงกว่า',//'只支持IE 5.01以上版本',
	'edit_raw'		: 'แก้ไขโค้ดต้นฉบับ',//'编辑源码',
	'plain_text_warn'	: 'ถ้าหากแปลงเป็นข้อความล้วนจะสูญเสียการจัดรูปแบบบางอย่าง แน่ใจหรือไม่ว่าคุณต้องการทำต่อไป?',//'转换为纯文本时将会遗失某些格式。\n您确定要继续吗？',
	'browser_update'	: 'บราวเซอร์ของคุณไม่รองรับคุณสมบัตินี้ กรุณาอัพเกรดเบราวเซอร์ของคุณให้เป็นเวอร์ชันล่าสุด',//'你的浏览器不支持此功能，请升级浏览器版本',
	'tips'			: 'เคล็ดลับ',//'小提示',
//	'show_tips'		: 'แสดงคำแนะนำ',//'友情提示',

//---------------------------
//static/image/editor/editor_function.js
// USED in: /source/admincp/admincp_feed.php
// USED in: /template/default/home/cpacecp_blog.htm
// USED in: /template/default/portal/portalcp_article.htm
// MOVED TO:
//static/js/editor_function.js
	'wysiwyg_only'		: 'การกระทำนี้มีเฉพาะในโหมด WYSIWYG',//'本操作只在多媒体编辑模式下才有效',

//---------------------------
//static/image/admincp/cloud/cloud.js
	'int_cloud_test'	: 'อินเตอร์เฟซการทดสอบเมฆแพลตฟอร์มอื่น ๆ',//'云平台其他接口测试',
	'int_roaming_test'	: 'การทดสอบการติดต่อข้ามเครือข่ายอื่น ๆ',//'漫游其他接口测试',
	'int_qq_test'		: 'การทดสอบการเชื่อมต่ออินเทอร์เน็ต QQ',//'QQ互联接口测试',

//---------------------------
//static/image/admincp/cloud/qqgroup.js
	'select_topic_to_push'	: 'เลือกอย่างน้อยหนึ่งหัวข้อสำหรับการผลักดันให้บทความ',//'请至少推送一条头条主题和一条列表主题',
	'select_item_to_push'	: 'เลือกอย่างน้อยหนึ่งรายการสำหรับการผลักดันให้บทความ',//'请至少推送一条信息到列表区域',
	'loading'		: 'กำลังโหลด...',//'加载中...',
	'push5reached'		: 'ผลักดันจำนวนโพสต์ได้ถึงห้าในสิทธิที่จะยกเลิกหมายเลขและลองอีกครั้ง',//'推送帖子已达到5条，请在右侧取消一些再重试。',
	'click_left'		: 'คลิกที่ด้านซ้าย',//'点击左侧',
	'push_to_list'		: 'จะผลักดันข้อมูลไปยังรายการ',//'将信息推送到列表',
	'wait_image_upload'	: 'อัปโหลดภาพกรุณารอสักครู่...',//'图片上传中，请稍后...',






//---------------------------
//upload/static/js/common_extra.js
//	'wait_please'		: 'กำลังโหลด...',//'请稍候...',

//--------------------------------
//static/js/calendar.js
//static/js/forum_calendar.js
//static/js/home_calendar.js

	'prev_month'	: 'เดือนก่อนหน้า',//'上一月',
	'next_month'	: 'เดือนต่อไป',//'下一月',
	'select_year'	: 'เลือกปี',//'点击选择年份',
	'select_month'	: 'เลือกเดือน',//'点击选择月份',
	'wday0'		: 'อา',//'日',
	'wday1'		: 'จ',//'一',
	'wday2'		: 'อ',//'二',
	'wday3'		: 'พ',//'三',
	'wday4'		: 'พฤ',//'四',
	'wday5'		: 'ศ',//'五',
	'wday6'		: 'ส',//'六',
	'month'		: 'เดือน',//'月',
	'today'		: 'วันนี้',//'今天',
	'hours'		: 'ชั่วโมง',//'点',
	'minutes'	: 'นาที',//'分',
	'ok'		: 'ตกลง',//'OK',

//--------------------------------
//static/js/common.js

	'open_new_win'		: 'เปิดหน้าต่างใหม่',//'在新窗口打开',
	'actual_size'		: 'ขนาดที่แท้จริง',//'实际大小',
	'close'			: 'ปิด',//'关闭',
	'wheel_zoom'		: 'ใช้สกลอร์เมาส์ เพื่อทำการขยาย/ย่อรูปภาพ',//'鼠标滚轮缩放图片',
	'reminder'		: 'แจ้งเตือน',//'提示信息',
	'submit'		: 'ส่ง',//'确定',
//	'submit'		: 'ส่ง',//'提交',
	'cancel'		: 'ยกเลิก',//'取消',
//	'cancel'		: 'ยกเลิก',//'取消',
	'wait_please'		: 'กำลังโหลด ...',//'请稍候...',
	'int_error'		: 'มีความผิดพลาดภายใน, ไม่สามารถแสดงเนื้อหาได้',//'内部错误，无法显示此内容',
	'flash_required'	: 'เนื้อหานี้ต้องการ Adobe Flash Player 9.0.124 หรือสูงกว่า',//'此内容需要 Adobe Flash Player 9.0.124 或更高版本',
	'flash_download'	: 'ดาวน์โหลด Flash Player',//'下载 Flash Player',
	'day1'			: '1 วัน',//'一天',
	'week1'			: '1 สัปดาห์',//'一周',
	'days7'			: '7 วัน',//'7 天',
	'days14'		: '14 วัน',//'14 天',
	'month1'		: '1 เดือน',//'一个月',
	'month3'		: '3 เดือน',//'三个月',
	'month6'		: '6 เดือน',//'半年',
	'year1'			: '1 ปี',//'一年',
	'custom'		: 'กำหนดเอง',//'自定义',
	'permanent'		: 'ถาวร',//'永久',
	'show_all_expr'		: 'แสดงอีโมฯ',//'显示所有表情',
	'page_prev'		: 'ก่อนหน้านี้',//'上页',
	'page_next'		: 'หน้าต่อไป',//'下页',
	'copy2clipboard'	: 'คัดลอกไปยังคลิปบอร์ด',//'点此复制到剪贴板',
	'enter_search_string'	: 'ค้นหาอะไร?',//'请输入搜索内容',
	'refresh_q&a'		: 'รีเฟรช Q&A',//'刷新验证问答',
	'refresh_code'		: 'รีเฟรชรหัส',//'刷新验证码',
	'code_invalid'		: 'รหัสความปลอดภัยไม่ถูกต้อง, กรุณาลองใหม่',//'验证码错误，请重新填写',
	'q&a_invalid'		: 'ตอบคำถามไม่ถูกต้อง, กรุณาลองใหม่',//'验证问答错误，请重新填写',
	'code_clipboard'	: 'โค๊ดถูกคัดลอกไปยังคลิปบอร์ดแล้ว',//'代码已复制到剪贴板',
	'enter_link_url'	: 'กรอกที่อยู่ URL',//'请输入链接地址',
	'enter_link_text'	: 'กรอกข้อความแทนลิงก์',//'请输入链接文字',
	'enter_image_url'	: 'กรอกที่อยู่รูปภาพ',//'请输入图片地址',
	'width_optional'	: 'กว้าง (ตัวเลือก)',//'宽(可选)',
	'height_optional'	: 'สูง (ตัวเลือก)',//'高(可选)',
	'narrow_screen'		: 'หน้าจอแคบ',//'切换到窄版',
	'wide_screen'		: 'จอแบบกว้าง',//'切换到宽版',
	'logging_wait'		: 'กำลังเข้าสู่ระบบ, โปรดรอสักครู่...',//'登录中，请稍后...',
	'notices_no'		: '[   ]',//'【　　　】',
	'notices_yes'		: '[ใหม่]',//'【新提醒】',
	'sec_after_win_closed'	: ' วินาที ก่อนหน้าต่างจะถูกปิด',//' 秒后窗口关闭',
	'sec_after_page_jump'	: ' วินาที ก่อนหน้านี้จะถูกเชื่อมต่อไปยังที่ใหม่',//' 秒后页面跳转',
	'jump_now'		: 'ข้ามไปยัง',//'立即跳转',
	'error_message'		: 'ข้อความผิดพลาด',//'错误信息',
	'ctrl_d_favorites'	: 'กดปุ่ม Ctrl + D เพิ่มไปยังรายการโปรด',//'请按 Ctrl+D 键添加到收藏夹',
	'non_ie_manually'	: 'ในกรณีที่ไม่ได้ใช้เบราวเซอร์ IE กรุณาตั้งหน้าแรกด้วยตัวเอง',//'非 IE 浏览器请手动将本站设为首页',
//--------------------------------
//static/js/common_diy.js

	'edit'			: 'แก้ไข',//'编辑',
	'warn_not_saved'	: 'คุณได้มีการแก้ไขข้อมูลแล้ว ถ้าหากออกจากหน้านี้ ข้อมูลทั้งหมดที่ถูกเปลี่ยนแปลงจะเสียหายได้.',//'您的数据已经修改,退出将无法保存您的修改。',
	'confirm_exit'		: 'ข้อมูลที่มีการเปลี่ยนแปลง/แก้ไขทั้งหมดจะสูญหายหากออกจากหน้านี้ คุณต้องการออกจากหน้านี้หรือไม่?',//'退出将不会保存您刚才的设置。是否确认退出？',
	'select_image_upload'	: 'กรุณาเลือกรูปภาพของคุณเพื่ออัปโหลด',//'请选择您要上传的图片',

//--------------------------------
//static/js/common_extra.js

	'copy_failed'		: 'การคัดลอกล้มเหลว กรุณาเลือก "อนุญาต"',//'复制失败，请选择“允许访问”',
//	'permanent'		: 'ถาวร',//'永久',
//	'open_new_win'		: 'เปิดในหน้าต่างใหม่',//'在新窗口打开',
//	'actual_size'		: 'ขนาดที่แท้จริง',//'实际大小',
//	'close'			: 'ปิด',//'关闭',
//	'wheel_zoom'		: 'ใช้ล้อเมาส์เพื่อซูมเข้า/ออกภาพ',//'鼠标滚轮缩放图片',
//	'reminder'		: 'เตือนความจำ',//'提示信息',
	'prev'			: 'ก่อน.',//'上一张',
	'next'			: 'ต่อไป',//'下一张',

//--------------------------------
//static/js/editor.js
//static/js/seditor.js

	'restore_size_edit'	: 'กลับสู่ขนาดปกติ',//'恢复编辑器大小',
	'full_screen_edit'	: 'ขยายขนาดเต็มหน้าจอ',//'全屏方式编辑',
	'current_length'	: 'เนื้อหาปัจจุบัน',//'当前长度',
	'bytes'			: 'ไบต์',//'字节',
	'system_limit'		: 'ขีดจำกัดระบบ',//'系统限制',
	'up_to'			: '~',//'到',
	'check_length'		: 'จำนวนตัวอักษร',//'字数检查',
	'data_restored'		: 'ข้อมูลถูกกู้คืน',//'数据已恢复',
	'data_saved'		: 'ข้อมูลถูกบันทึก',//'数据已保存',
	'clear_all_sure'	: 'คุณแน่ใจว่าจะล้างเนื้อหานี้ทั้งหมด?',//'您确认要清除所有内容吗？',
	'hide_content'		: 'ซ่อนเนื้อหา',//'请输入要隐藏的信息内容',
	'free_content'		: 'หากคุณไม่ได้กำหนดราคาการโพสต์ข้อมูลที่ป้อนจะสามารถมองเห็นเสียค่าใช้จ่ายก่อนที่จะซื้อเนื้อหาที่,',//'如果您设置了帖子售价，请输入购买前免费可见的信息内容',
	'when_thread_replied'	: 'แสดงเฉพาะเมื่อผู้ใช้ตอบกระทู้นี้ ',//'只有当浏览者回复本帖时才显示',
	'when_points_more'	: 'แสดงเฉพาะเมื่อผู้ใช้ชี้เป็นมากกว่า ',//'只有当浏览者积分高于',
	'when_show'		: 'เมื่อที่จะแสดง',//'时才显示',
	'table_rows'		: 'แถวของตาราง',//'表格行数',
	'table_columns'		: 'คอลัมน์ตาราง',//'表格列数',
	'table_width'		: 'ความกว้างของตาราง: ',//'表格宽度: ',
	'bg_color'		: 'สีพื้นหลัง',//'背景颜色',
	'table_intro0'		: 'เคล็ดลับการเขียนตารางด่วน',//'快速书写表格提示',
	'table_intro1'		: '&quot;[tr=color]&quot; กำหนดสีพื้นหลังแถว<br />&quot;[td=Width]&quot; กำหนดความกว้างของคอลัมน์<br />&quot;[td=Column_Span,Row_Span,Width]&quot; กำหนดช่วงแถว/คอลัมน์และความกว้าง<br /><br />ตัวอย่างการเขียนตารางได้อย่างรวดเร็ว: ',//'“[tr=颜色]” 定义行背景<br />“[td=宽度]” 定义列宽<br />“[td=列跨度,行跨度,宽度]” 定义行列跨度<br /><br />快速书写表格范例：',
	'table_intro2'		: '[table]<br />ชื่อ:|Discuz!<br />รุ่น:|X2.0<br />[/table]',//'[table]<br />Name:|Discuz!<br />Version:|X1<br />[/table]',
	'table_intro3'		: 'ใช้ &quot;|&quot; สำหรับแถวที่แยกจากกัน, หากมีการ &quot;|&quot; ตัวอักษรในข้อมูลที่, แทนที่มันด้วย &quot;\\|&quot;, แถวแยกจากกันด้วย &quot;\\n&quot;.',//'用“|”分隔每一列，表格中如有“|”用“\\|”代替，换行用“\\n”代替。',
	'audio_url'		: 'ป้อน URL ของไฟล์เพลง',//'请输入音乐文件地址',
	'video_url'		: 'ป้อน URL ของไฟล์วิดีโอ',//'请输入视频地址',
	'auto_play'		: 'อัตโนมัติ?',//'是否自动播放',
	'flash_url'		: 'กรุณาใส่ URL ของไฟล์ Flash ',//'请输入 Flash 文件地址',
	'enter_please'		: 'กรุณากรอกตัวอักษร',//'请输入第',
	'nth_parameter'		: 'พารามิเตอร์',//' 个参数',
	'font'			: 'ตัวอักษร',//'字体',
	'full_screen'		: 'เต็มหน้าจอ',//'全屏',
	'restore_size'		: 'คืนค่าขนาด',//'恢复',
	'general'		: 'โหมดทั่วไป',//'常用',
	'simple'		: 'โหมดขั้นสูง',//'高级',
	'bad_browser'		: 'เบราว์เซอร์ของคุณไม่สนับสนุนคุณลักษณะนี้',//'你的浏览器不支持此功能',
	'click_autosave_enable'	: 'คลิกที่นี่เพื่อเปิดใช้งานอัตโนมัติที่ช่วยประหยัด',//'点击开启自动保存',
	'autosave_enable'	: 'การเปิดใช้งานอัตโนมัติที่ช่วยประหยัด',//'开启自动保存',
	'autosave_disable'	: 'ปิดการใช้งานอัตโนมัติที่ช่วยประหยัด',//'点击关闭自动保存',
	'autosave_enabled'	: 'ข้อมูลอัตโนมัติที่ช่วยประหยัดการใช้งาน',//'数据自动保存已开启',
	'autosave_disabled'	: 'ข้อมูลอัตโนมัติปิดการใช้งานที่ประหยัด',//'数据自动保存已关闭',
	'data_saved_at'		: 'ข้อมูลที่บันทึกไว้ที่',//'数据已于',
	'saved_time'		: '',//NOT REQUIRED IN ENGLISH!//'保存',
	'sec_before_saving'	: 'วินาทีก่อนที่จะบันทึกอัตโนมัติ',//'秒后保存',
	'insert_quote'		: 'แทรกคำพูด',//'请输入要插入的引用',
	'insert_code'		: 'ใส่รหัส',//'请输入要插入的代码',
//	'enter_image_url'	: 'ป้อน URL ของภาพ',//'请输入图片地址',
//	'width_optional'	: 'ความกว้าง (จำเป็น)',//'宽(可选)',
//	'height_optional'	: 'ความสูง (จำเป็น)',//'高(可选)',
	'enter_item_list'	: 'ป้อนรายการรายการ.\r\nเว้นว่างไว้หรือคลิกยกเลิก.',//'输入一个列表项目.\r\n留空或者点击取消完成此列表.',
//	'enter_link_url'	: 'ป้อน URL การเชื่อมโยง',//'请输入链接地址',
//	'enter_link_text'	: 'ป้อนข้อความการเชื่อมโยง',//'请输入链接文字',
//	'insert_quote'		: 'แทรกคำพูด',//'请输入要插入的引用',
	'width'			: 'ความกว้าง',//'宽',
	'height'		: 'ความสูง',//'高',
	'audio_support'		: 'ที่รองรับไฟล์ WMA, MP3, RA, RM, และรูปแบบของเพลงอื่น ๆ<br />ตัวอย่างเช่น: http://server/audio.wma',//'支持 wma mp3 ra rm 等音乐格式<br />示例: http://server/audio.wma',
	'video_support'		: 'การสนับสนุนสำหรับ Youku, Potatoes, 56, 6, Cool video และสถานีวิดีโออื่น ๆ<br />ได้รับการสนับสนุน WMV AVI RMVB MOV SWF FLV รูปแบบวิดีโอ.<br />ตัวอย่างเช่น: http://server/movie.wmv',//'支持优酷、土豆、56、酷6等视频站的视频网址<br />支持 wmv avi rmvb mov swf flv 等视频格式<br />示例: http://server/movie.wmv',
	'flash_support'		: 'ที่สนับสนุนรูปแบบ Flash: swf flv<br />ตัวอย่างเช่น: http://server/flash.swf',//'支持 swf flv 等 Flash 网址<br />示例: http://server/flash.swf',
	'paste_from_word'	: 'วางเนื้อหาจากโปรแกรม Word',//'从 Word 粘贴内容',
	'paste_word_tip'	: 'กรุณาใช้ทางลัด (Ctrl + V) เพื่อวางเนื้อหาจากเอกสาร Word',//'请通过快捷键(Ctrl+V)把 Word 文件中的内容粘贴到上',
//--------------------------------
//static/js/forum.js

	'del_thread_sure'	: 'คุณแน่ใจหรือว่าต้องการที่จะลบกระทู้นี้จากกระทู้ร้อน?',//'您确认要把此主题从热点主题中移除么？',
	'there_are'		: 'มี ',//'有',
//	'new_reply_exists'	: ' การตอบกระทู้ใหม่ในหัวข้อที่ให้คลิกเพื่อดู',//'条存在新回复的主题，点击查看',
	'new_reply_exists'	: 'หัวข้อที่มีการตอบกระทู้ใหม่คลิกที่ดู',//'有新回复的主题，点击查看',
//--------------------------------
//static/js/forum_google.js

	'search_net'	: 'ค้นหาในอินเทอร์เน็ต',//'网页搜索',
	'search_site'	: 'ค้นหาในเว็บไซต์',//'站内搜索',
	'search'	: 'ค้นหา',//'搜索',

//--------------------------------
//static/js/forum_moderate.js

	'choose_tread'	: 'เลือกด้ายถึงปานกลาง',//'请选择需要操作的帖子',

//--------------------------------
//static/js/forum_post.js

	'internal_error'	: 'ข้อผิดพลาดภายในเซิร์ฟเวอร์',//'内部服务器错误',
	'upload_ok'		: 'อับโหลดไฟล์เสร็จเรียบร้อย',//'上传成功',
	'ext_not_supported'	: 'รูปแบบไฟล์หรือนามสกุลนี้ไม่สนับสนุน',//'不支持此类扩展名',
	'sorry_ext_not_supported'	: 'รูปแบบไฟล์หรือนามสกุลนี้ไม่สนับสนุน',//'对不起，不支持上传此类扩展名的附件。',
	'illegal_image_type'	: 'รูปแบบไฟล์หรือนามสกุลนี้ไม่สนับสนุน',//'图片附件不合法',
	'can_not_save_attach'	: 'ไม่สามารถบันทึกไฟล์แนบ',//'附件文件无法保存',
	'invalid_file'		: 'ไม่มีรูปแบบไฟล์ที่ถูกต้อง',//'没有合法的文件被上传',
	'illegal_operation'	: 'ดำเนินการผิดพลาด',//'非法操作',
//	'current_length'	: 'Current Length',//'当前长度',
//	'bytes'			: 'bytes',//'字节',
//	'system_limit'		: 'System limit',//'系统限制',
//	'up_to'			: 'to',//'到',
//	'bytes'			: 'bytes',//'字节',
//	'check_length'		: 'Word Count',//'字数检查',
	'enter_content'		: 'กรุณาใส่ชื่อกระทู้หรือเนื้อหา',//'请完成标题或内容栏',
	'select_category'	: 'กรุณาเลือกประเภทของกระทู้',//'请选择主题对应的分类',
	'select_category_info'	: 'กรุณาเลือกหมวดหมู่ของกระทู้',//'请选择主题对应的分类信息',
	'title_long'		: 'ชื่อกระทู้ของคุณมีความยาวมากกว่า 255 ตัวอักษร',//'您的标题超过 80 个字符的限制',
	'content_long'		: 'ความยาวของเนื้อหาไม่ตรงกับความต้องการ.\n\n',//'您的帖子长度不符合要求。\n\n',
//	'bytes'			: 'ไบต์',//'字节',
//	'system_limit'		: 'ขีด จำกัด ของระบบ',//'系统限制',
//	'up_to'			: 'ถึง',//'到',
	'ignore_pending_attach'	: 'มีสิ่งที่แนบมารอแล้วคุณแน่ใจว่าจะไม่สนใจมัน?',//'您有等待上传的附件，确认不上传这些附件吗？',
	'still_uploading'	: 'สิ่งที่แนบมาบางส่วนยังคงอัปโหลดกรุณารอสักครู่ หัวข้อที่จะถูกเผยแพร่โดยอัตโนมัติหลังจากที่ไฟล์ที่ถูกอัปโหลด...',//'您有正在上传的附件，请稍候，上传完成后帖子将会自动发表...',
//	'q&a_invalid'			: 'Wrong answer, please try again',//'验证问答错误，请重新填写',
//	'code_invalid'			: 'Wrong security code, please try again',//'验证码错误，请重新填写',
	'no_data_recover'	: 'ไม่มีข้อมูลที่สามารถกู้คืนได้!',//'没有可以恢复的数据！',
	'content_overwrite'	: 'เนื้อหาด้ายปัจจุบันจะถูกแทนที่ด้วยการดำเนินการนี้คุณแน่ใจว่าจะเรียกคืนข้อมูลได้หรือไม่',//'此操作将覆盖当前帖子内容，确定要恢复数据吗？',
	'upload_finished'	: 'อัปโหลดจะเสร็จสิ้น!',//'附件上传完成！',
	'successfull'		: 'ที่ประสบความสำเร็จ:',//'成功',
	'failed'		: 'ล้มเหลว:',//'失败',
	'ones'			: 'รายการ',//'个',
	'uploading'		: 'การอัปโหลด...',//'上传中...',
	'select_image_files'	: 'Select image files',//'请选择图片文件',
	'delete'		: 'ลบ',//'删除',
//	'cancel'		: 'Cancel',//'取消',
	'contains'		: 'มี ',//'包含',
	'img_attached_num'	: ' รูปที่แนบมา',//'个图片附件',
	'files attached_num'	: ' ไฟล์ที่แนบมา',//'个附件',
	'images'		: 'รูปภาพ',//'图片',
	'attachments'		: 'สิ่งที่แนบมา',//'附件',
	'upload_failed'		: 'อัปโหลดล้มเหลว',//'上传失败',

	'attach_big'		: 'ขนาดไฟล์แนบเกินวงเงินอนุญาต',//'服务器限制无法上传那么大的附件',
	'attach_group_big'	: 'กลุ่มผู้ใช้ทั้งหมดขนาดของไฟล์ที่แนบมาเกินขีด จำกัด ได้รับอนุญาต',//'用户组限制无法上传那么大的附件',
	'attach_type_big'	: 'ไฟล์ประเภทนี้รวมขนาดไฟล์แนบเกินวงเงินอนุญาต',//'文件类型限制无法上传那么大的附件',
	'attach_daily_big'	: 'ขนาดไฟล์แนบรายวันรวมเกินขีด จำกัด ได้รับอนุญาต',//'本日已无法上传更多的附件',
	'validating_q&a'	: 'การตรวจสอบ Q&A กรุณารอสักครู่',//'验证问答校验中，请稍后',
	'validating_code'	: 'การตรวจสอบรหัสที่กรุณารอสักครู่',//'验证码校验中，请稍后',
	'attach_type_disabled'	: 'ประเภทสิ่งที่แนบมานี้ถูกปิดใช้งาน',//'附件类型被禁止',
	'attach_max'		: 'ไม่ได้มีขนาดใหญ่กว่า',//'不能超过',
	'vote_max_reached'	: 'ถึงจำนวนสูงสุดจากการโหวต: ',//'已达到最大投票数',
	'no_remote_attach'	: 'ขออภัยไม่มีสิ่งที่แนบจากระยะไกล',//'抱歉，暂无远程附件',
	'delete_post_sure'	: 'แน่ใจหรือว่าต้องการลบบทความนี้?',//'确定要删除该帖子吗？',
//--------------------------------
//static/js/forum_viewthread.js

	'best_answer_sure'	: 'คุณแน่ใจหรือว่าคุณต้องการกำหนดโพสต์นี้เป็น"คำตอบที่ดีที่สุด"?',//'您确认要把该回复选为“最佳答案”吗？',
//	'title_long'		: 'Title length exceeds the limit of 255 characters',//'您的标题超过 255 个字符的限制',
//	'content_long'		: 'The content length does not meet the requirements.\n\nCurrent Length '//'您的帖子长度不符合要求。\n\n当前长度',
//	'bytes'			: 'bytes',//'字节',
//	'system_limit'		: 'System limit',//'系统限制',
//	'up_to'			: 'to',//'到',
	'premoderated'		: 'การตอบกลับทั้งหมดหมวดหมู่นี้จะต้องผ่านการตรวจสอบ โพสต์ของคุณ wll จะแสดงหลังจากการตรวจสอบ',//'本版回帖需要审核，您的帖子将在通过审核后显示',
//	'credit_confirm1'	: 'This costs ',//'下载积分将',
	'credit_confirm1'	: 'ค่าใช้จ่ายในการดาวน์โหลดเป็น ',//'下载需要消耗',
	'credit_confirm2'	: ' คะแนน, คุณแน่ใจที่จะดาวน์โหลดหรือไม่?',//'，您是否要下载？',
	'thread_to_clipboard'	: 'URL ที่กระทู้ถูกคัดลอกไปที่คลิปบอร์ด',//'帖子地址已经复制到剪贴板',
	'click_to_enlarge'	: 'คลิกเพื่อดูภาพขยาย',//'点击放大',
	'notify_on_reply'	: 'ได้รับการแจ้งตอบกลับ',//'接收回复通知',
	'notify_on_reply_cancel'	: 'ยกเลิกการแจ้งเตือนการตอบกลับ',//'取消回复通知',
	'share_connection_failed'	: 'การเชื่อมต่อกับส่วนแบ่งการล้มเหลวโปรดลองอีกครั้งในภายหลัง',//'分享服务连接失败，请稍后再试',
	'qq_bind'		: 'กรุณาผูกบัญชีของคุณ QQ',//'请先绑定QQ账号',
	'quote_by'	: 'โดยอ้าง .*? ใน .*? รหัส',//'本帖最后由 .*? 于 .*? 编辑',
	'copy_code'	: 'คัดลอกโค้ด',//'复制代码',

//--------------------------------
//static/js/home.js
	'day'			: 'วัน',//'日',
	'category_empty'	: 'ชื่อประเภทไม่สามารถว่างเปล่า!',//'分类名不能为空！',

//--------------------------------
//static/js/home_ajax.js

//	'close'			: 'Close',//'关闭',
//	'wait_please'		: 'Loading ...',//'请稍候...',

//--------------------------------
//static/js/home_blog.js

	'title_length_invalid'	: 'ชื่อเรื่องความยาวไม่ตรงตามความต้องการ ควรจะมี 1 ~ 80 ตัวอักษร',//'标题长度(1~80字符)不符合要求',

//--------------------------------
//static/js/home_common.js

	'show_orig_image'	: 'แสดงภาพต้นฉบับในหน้าต่างใหม่',//'点击图片，在新窗口显示原始尺寸',
	'continue_sure'		: 'คุณแน่ใจว่าจะไปหรือไม่?',//'您确定要执行本操作吗？',
	'select_item'		: 'กรุณาเลือกรายการที่ต้องการทำงานด้วย',//'请选择要操作的对象',
	'image_url_invalid'	: 'URL ของภาพที่ไม่ถูกต้อง',//'图片地址不正确',
	'audio_url_invalid'	: 'URL เสียงที่ไม่ถูกต้องไม่สามารถว่างเปล่า',//'音乐地址错误，不能为空',

//!!!!! MayBe wrap this names!!
	'collapse'		: 'ยุบ',//'收起',
	'expand'		: 'ขยาย',//'展开',

//--------------------------------
//static/js/home_friendselector.js

	'select_max'		: 'คุณสามารถเลือกได้ถึง',//'最多只允许选择',
	'users'			: 'ผู้ใช้',//'个用户',
	'allready_exists'	: 'ที่มีอยู่แล้ว',//'已经存在',

//--------------------------------
//static/js/home_manage.js

	'you_friends_now'	: 'คุณเป็นเพื่อนตอนนี้คุณสามารถ ',//'你们现在是好友了，接下来，您还可以：',
	'leave_message'		: 'ฝากข้อความไว้',//'给TA留言',
	'or'			: 'หรือ',//'或者',
	'send_greeting'		: 'ส่งคำอวยพร',//'打个招呼',
//	'collapse'			: 'Collapse',//'收起',
	'reply'			: 'ตอบ',//'回复',
	'comment'		: 'คิดเห็น',//'评论',
	'close_list'		: 'ปิดรายการ',//'收起列表',
	'more_feeds'		: 'ฟีดข้อมูลเพิ่มเติม',//'更多动态',
//	'day'			: 'Day',//'日',

//--------------------------------
//static/js/home_uploadpic.js

	'image_type_invalid'	: 'ขออภัยภาพที่มีการขยายดังกล่าวไม่ได้รับการสนับสนุน',//'对不起，不支持上传此类扩展名的图片',
	'insert_to_content'	: 'คลิกที่นี่เพื่อแทรกลงในเนื้อหาที่ตำแหน่งของเคอร์เซอร์ปัจจุบัน',//'点击这里插入内容中当前光标的位置',
	'insert'		: 'แทรก',//'插入',
//	'delete'		: 'Delete',//'删除',
	'image_description'	: 'คำอธิบาย',//'图片描述',
//	'upload_ok'		: 'Uploaded Successfully',//'上传成功',
//	'upload_failed'		: 'Upload Failed',//'上传失败',
	'uploading_wait'	: 'อัปโหลดโปรดรอสักครู่',//'上传中，请等待',
	'retry'			: 'ลองอีกครั้ง',//'重试',

//--------------------------------
//static/js/portal.js

	'delete_sure'		: 'คุณแน่ใจที่จะลบข้อมูลนี้หรือไม่?',//'您确定要删除该数据吗？',
	'ignore_sure'		: 'คุณแน่ใจหรือไม่ที่จะไม่สนใจข้อมูลนี้?',//'您确定要屏蔽该数据吗？',
	'to'			: 'ไป',//'到',

	'choose_block'		: 'เลือกบล็อก',//'选择模块',
	'blocks_found1'		: 'ถูกค้นพบ',//'找到',
	'blocks_found2'		: 'บล็อกที่สอดคล้องกัน',//'个相应的模块',
	'blocks_not_found'	: 'ไม่มีบล็อกที่สอดคล้องกัน',//'没有相应的模块',
	'select_block'		: 'กรุณาเลือกบล็อก!',//'请选择一个模块！',
	'show_settings'		: 'แสดงการตั้งค่า',//'展开设置项',
	'hide_settings'		: 'ซ่อนการตั้งค่า',//'收起设置项',
	'block_name_empty'	: 'ชื่อที่ถูกบล็อกไม่สามารถว่างเปล่า',//'模块标识不能为空',
	'block_convert_sure'	: 'คุณแน่ใจหรือว่าคุณต้องการแปลงบล็อกตั้งแต่วันที่ประเภท',//'你确定要转换模块的类型从',
	'back'			: 'เข้ากลับ',//'返回',
	'settings_expand'	: 'ขยายการตั้งค่า',//'展开设置项',
	'settings_hide'		: 'ซ่อนการตั้งค่า',//'收起设置项',
	'custom_content_error'	: 'ข้อผิดพลาดในเนื้อหาที่กำหนดเอง! โค้ด HTML: ',//'自定义内容错误，',
	'html_error'		: 'โค้ด HTML: ',//'HTML代码：',
	'tags_not_match'	: ' - แท็กไม่ตรงกับ',//' 标签不匹配',

//--------------------------------
//static/js/portal_diy.js

	'choose_style'		: 'เลือกรูปแบบ',//'选择样式',
	'style'			: 'รูปแบบ',//'样式',
	'style1'		: 'รูปแบบ 1',//'样式1',
	'style2'		: 'รูปแบบ 2',//'样式2',
	'style3'		: 'รูปแบบ 3',//'样式3',
	'style4'		: 'รูปแบบ 4',//'样式4',
	'style5'		: 'รูปแบบ 5',//'样式5',
	'style6'		: 'รูปแบบ 6',//'样式6',
	'style7'		: 'รูปแบบ 7',//'样式7',
	'no_border'		: 'No border frame',//'无边框框架',
	'no_border_no_margin'	: 'No border, no margin',//'无边框且无边距',

//	'choose_style'		: 'Choose a Style',//'选择样式',
	'title'			: 'ชื่อเรื่อง',//'标题',
//	'delete'		: 'Delete',//'删除',
	'attribute'		: 'คุณลักษณะ',//'属性',
	'data'			: 'ข้อมูล',//'数据',
	'update'		: 'อัพเดต',//'更新',
	'export'		: 'ส่งออก',//'导出',
	'repeat'		: 'ทำซ้ำ',//'平铺',
	'no_repeat'		: 'ไม่มีการทำซ้ำ',//'不平铺',
	'repeat_x'		: 'ทำซ้ำแนวนอน',//'横向平铺',
	'repeat_y'		: 'ทำซ้ำในแนวตั้ง',//'纵向平铺',
	'no_style'		: 'โดยไม่ต้องมีสไตล์',//'无样式',
	'solid_line'		: 'เส้นทึบ',//'实线',
	'dotted_line'		: 'จุดไข่ปลา',//'点线',
	'dashed_line'		: 'เส้นประ',//'虚线',
//	'font'			: 'Font',//'字体',
	'link'			: 'ลิงก์',//'链接',
	'border'		: 'เส้นขอบ',//'边框',
	'size'			: 'ขนาด',//'大小',
	'color'			: 'สี',//'颜色',
	'separate_config'	: 'Config แยก',//'分别设置',
	'right'			: 'ขวา',//'右',
	'bottom'		: 'ล่าง',//'下',
	'top'			: 'บน',//'上',
	'left'			: 'ซ้าย',//'左',
	'margin'		: 'ขอบ',//'外边距',
	'padding'		: 'แพ็ดดิ้ง',//'内边距',
//	'background_color'	: 'Background Color',//'背景颜色',
	'bg_image'		: 'รูปภาพพื้นหลัง',//'背景图片',
	'class'			: 'ระดับที่กำหนด',//'指定class',
	'block'			: 'กลุ่ม',//'模块',
	'frame'			: 'เฟรม',//'框架',
//	'edit'			: 'Edit',//'编辑',
//	'style'			: 'Style',//'样式',
//	'close'			: 'Close',//'关闭',
//	'submit'		: 'Submit',//'确定',
//	'cancel'		: 'Cancel',//'取消',
//	'tile'			: 'Tile',//'平铺',
//	'no_tile'		: 'No tile',//'不平铺',
//	'tile_hor'		: 'Horizontal Tile',//'横向平铺',
//	'tile_ver'		; 'Vertical Tile',//'纵向平铺',
	'onclick'		: 'onClick',//'点击',
	'onmouseover'		: 'onMouseover',//'滑过',
	'switch_type'		: 'สลับรูปแบบ',//'切换类型',
//	'title'			: 'Title',//'标题',
//	'link'			: 'Link',//'链接',
	'image'			: 'ภาพ',//'图片',
	'position'		: 'ตำแหน่ง',//'位置',
	'align_left'		: 'ชิดซ้าย',//'居左',
	'align_right'		: 'ชิดขวา',//'居右',
	'offset'		: 'ออฟเซ็ท',//'偏移量',
//	'font'			: 'Font',//'字体',
//	'size'			: 'Size',//'大小',
//!!! mainly the same as 'color' !!!!!!
//	'colour'		: 'Colour',//'色',
	'add_new_title'		: 'เพิ่มชื่อเรื่องใหม่',//'添加新标题',
//	'edit'			: 'Edit',//'编辑',
//	'title'			: 'Title',//'标题',
//	'close'			: 'Close',//'关闭',
//	'submit'		: 'Submit',//'确定',
//	'cancel'		: 'Cancel',//'取消',
	'delete_this_sure'	: 'คุณแน่ใจที่จะลบมันได้หรือไม่ มันไม่สามารถเรียกคืนถ้าคุณลบมัน',//'您确实要删除吗,删除以后将不可恢复',
	'loading_content'	: 'กำลังโหลดข้อมูล...',//'正在加载内容...',
	'modified_import'	: 'คุณได้แก้ไขบางส่วนกรุณานำเข้าหลังจากที่คุณบันทึกไว้มิฉะนั้นข้อมูลที่นำเข้าจะไม่รวมถึงการเปลี่ยนแปลงของเวลานี้',//'您已经做过修改，请保存后再做导出，否则导出的数据将不包括您这次所做的修改。',
	'total'			: 'ทั้งหมด ',//'共',
	'blocks'		: 'บล็อก',//'个模块',
	'updating_the'		: 'การปรับปรุง #',//'正在更新第',
//	'ones'			: 'ones',//'个',
	'done'			: 'ทำ',//'已完成',
	'start_updating'	: 'เริ่มการอัปเดต...',//'开始更新...',
	'update_block_data'	: 'การอัปเดตข้อมูลบล็อก',//'更新模块数据',
	'clear_diy_sure'	: 'คุณแน่ใจที่จะล้างข้อมูลในปัจจุบันหน้า DIY? มันไม่สามารถเรียกคืนถ้าคุณล้างมัน',//'您确实要清空页面上所在DIY数据吗,清空以后将不可恢复',
	'frame_not_found'	: 'คำเตือน : ไม่พบกรอบโปรดเพิ่มเฟรม',//'提示：未找到框架，请先添加框架。',
//	'warn_not_saved'	: 'You have modified the data. If you exit, all the changes will be lost.',//'您的数据已经修改,退出将无法保存您的修改。',
	'apply_all_pages'	: 'นำไปใช้กับหน้าเว็บทั้งหมดนี้ประเภท',//'应用于此类全部页面',
	'apply_current_page'	: 'นำไปใช้กับหน้าปัจจุบัน',//'只应用于本页面',
	'save_temp_sure'	: 'บันทึกข้อมูลชั่วคราว<br />คลิกที่"ส่ง"เพื่อบันทึกข้อมูลชั่วคราวให้คลิก"ยกเลิก"เพื่อลบข้อมูลชั่วคราว',//'是否保留暂存数据？<br />按确定按钮将保留暂存数据，按取消按钮将删除暂存数据。',
	'save_temp'		: 'บันทึกข้อมูลชั่วคราว',//'保留暂存数据',
	'revert_last_saved'	: 'คุณแน่ใจหรือว่าต้องการกลับไปรุ่นก่อนหน้าของผลการบันทึกไว้?',//'您确定要恢复到上一版本保存的结果吗？',
	'continue_temp_sure'	: 'ดำเนินการต่อไป DIY กับข้อมูลชั่วคราว?',//'是否继续暂存数据的DIY？',
//	'warn_not_saved'	: 'You have modified the data. If you exit, all the changes will be lost.',//'您的数据已经修改,退出将无法保存您的修改。',
	'update_completed'	: 'การปรับปรุงจะเสร็จสมบูรณ์',//'已更新完成。',
	'tab_label'		: 'ป้ายชื่อแท็บ',//'tab标签',
	'temp_action'		: 'คลิกที่ปุ่ม"Continue"ในการโหลดข้อมูลชั่วคราวในรูปแบบปัจจุบัน<br />คลิกที่ปุ่ม"ลบ"สำหรับลบข้อมูลชั่วคราว',//'按继续按钮将打开暂存数据并DIY，<br />按删除按钮将删除暂存数据。',
	'continue'		: 'ต่อ',//'继续',

//--------------------------------
//static/js/qshare.js
	'from_tencent'		: 'มาจาก Tencent microblog แพลตฟอร์มแบบเปิด',//"\u6211\u6765\u81EA\u4E8E\u817E\u8BAF\u5FAE\u535A\u5F00\u653E\u5E73\u53F0",

//--------------------------------
//static/js/register.js

	'username_invalid'	: 'ชื่อที่คุณใช้มีตัวอักษรหรือคำที่ห้ามใช้',//'用户名包含敏感字符',
	'username_short'	: 'ชื่อผู้ใช้สั้นเกินไป อย่างน้อย 3 ตัวอักษร',//'用户名小于 3 个字符',
	'username_long'		: 'ชื่อผู้ใช้ยาวเกินไป สูงสุด 15 ตัวอักษร',//'用户名超过 15 个字符',
	'passwords_not_equal'	: 'รหัสผ่านทั้งสองช่องไม่ตรงกัน',//'两次输入的密码不一致',
	'email_invalid'		: 'อีเมลที่คุณใช้มีตัวอักษรหรือคำที่ห้ามใช้',//'Email 包含敏感字符',
	'invite_code_invalid'	: 'โค้ดเชิญชวนที่คุณใช้มีตัวอักษรหรือคำที่ห้ามใช้',//'邀请码包含敏感字符',
	'password_fill'		: 'กรุณากรอกรหัสผ่าน',//'请填写密码',
	'password_again'	: 'Pโปรดใส่รหัสผ่านอีกครั้ง',//'请再次输入密码',
	'email_fill'		: 'กรุณาป้อนที่อยู่อีเมล',//'请输入邮箱地址',

//--------------------------------
//static/js/smilies.js

//---------------------------
//static/js/threadsort.js

	'select_please'		: 'กรุณาเลือก',//'请选择',
	'required_fill'		: 'จำเป็นต้องใช้ข้อมูลเหล่านี้',//'必填项目没有填写',
	'select_next_level'	: 'จำเป็นต้องใช้ข้อมูลเหล่านี้',//'请选择下一级',
	'numeric_invalid'	: 'กรอกตัวเลขไม่ถูกต้อง',//'数字填写不正确',
	'email_invalid'		: 'ที่อยู่อีเมลไม่ถูกต้อง',//'邮件地址不正确',
	'text_too_long'		: 'กรอกข้อมูลในโครงการยาวเกินไป',//'填写项目长度过长',
	'value_is_greater'	: 'สูงกว่าค่าที่กำหนดไว้',//'大于设置最大值',
	'value_is_less'		: 'น้อยกว่าค่าที่กำหนดไว้',//'小于设置最小值',
//--------------------------------
//static/js/space_diy.js

//	'delete'		: 'Delete',//'删除',
//	'attribute'		: 'Attribute',//'属性',
	'save_js'		: 'บันทึกจาวาสคริปต์หลังจากที่การแสดง',//'javascript脚本保存后显示',
	'settings'		: 'ตั้งค่า',//'设置',

//--------------------------------
//static/js/upload.js

	'file_not_supported'	: 'ขออภัย, รูปไฟล์ไม่รองรับ',//'对不起，不支持上传此类文件',
//	'uploading'		: 'Uploading...',//'上传中...',

//-------------------------------------
//source/function/function_admincp.php
	'version_uptodate'	: 'ขณะนี้คุณใช้ up - to - date รุ่นของ Discuz! โปรแกรม โปรดดูที่เคล็ดลับต่อไปนี้เพื่อทำการอัพเกรดเวลาที่เหมาะสม',

//-------------------------------------
//api/manyou/cloud_iframe.js
	'add_operation'		: 'เพิ่มไปยังการดำเนินงานทั่วไป',//'&#28155;&#21152;&#21040;&#24120;&#29992;&#25805;&#20316;',

//-------------------------------------
//	''	: '',//'',

'fiction'	: ''

};
