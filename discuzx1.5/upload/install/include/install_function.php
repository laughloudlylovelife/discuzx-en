...
<div class="main" style="margin-top:-123px;">
	...
	<div class="btnbox marginbot">
		...
		<input type='hidden' name='language' value='$language' /><!-- vot -->
...
function upg_comsenz_stats() {
	...
	if(getgpc('addfounder_contact','P')) {
		...
		$founder_contact = str_replace(array("\n","\t"), array('<br>','&nbsp;&nbsp;&nbsp;&nbsp;'), $founder_contact);
			...
			echo '</form>';
			echo '<p style="text-align:right"><input type="button" style="padding: 2px;" onclick="window.location=\'index.php?method=ext_info&skip=1&language='.$language.'\'" value="'.lang('skip_current').'" /></center></p>'; //vot
			echo '</div>';
		show_footer();
	}
}
...
function import_diy($importfile, $primaltplname, $targettplname) {
	...
	if ($diycontent) {
		...
		if (!empty($diycontent['blockdata'])) {
			...
			unset($diycontent['blockdata']); //vot
		}
