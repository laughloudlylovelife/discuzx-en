...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt) {
	...
	var menuObj = $(menuid);
/*vot*/	confirmtxt = confirmtxt ? confirmtxt : lng['submit'];
...
/*vot*/	str = lng['flash_required']+'<br /><a href="http://www.adobe.com/go/getflashplayer/" target="_blank"><img src="http://www.adobe.com/images/shared/download_buttons/get_flash_player.gif" alt="'+lng['flash_download']+'" /></a>';
	}
	return str;
}
...
/*vot*/		menu.innerHTML = !nocover ? '<div class="zoominner"><p id="' + menuid + '_ctrl"><span class="y"><a href="' + zimg + '" class="imglink" target="_blank" title="'+lng['open_new_win']+'">'+lng['open_new_win']+'</a><a href="javascipt:;" id="' + menuid + '_adjust" class="imgadjust" title="'+lng['actual_size']+'">'+lng['actual_size']+'</a><a href="javascript:;" onclick="hideMenu()" class="imgclose" title="'+lng['close']+'">'+lng['close']+'</a></span>'+lng['wheel_zoom']+'</p><div align="center" onmousedown="zoomclick=1" onmousemove="zoomclick=2" onmouseup="if(zoomclick==1) hideMenu()"><img id="' + zoomid + '" src="' + zimg + '" width="' + w + '" height="' + h + '" w="' + imgw + '" h="' + imgh + '"></div></div>' :
/*vot*/				'<div class="popupmenu_popup" style="width:auto" onmousedown="zoomclick=1" onmousemove="zoomclick=2" onmouseup="if(zoomclick==1) hideMenu()"><span class="right y"><a href="javascript:;" onclick="hideMenu()" class="flbc" style="width:20px;margin:0 0 2px 0">'+lng['close']+'</a></span>'+lng['wheel_zoom']+'<img id="' + zoomid + '" src="' + zimg + '" width="' + w + '" height="' + h + '" w="' + imgw + '" h="' + imgh + '"></div>';
			$('append_parent').appendChild(menu);
			if(!nocover) {
...
//vot: var colortexts MOVED TO lang_js.js
		var str = '';
		for(var i = 0; i < 40; i++) {
/*vot*/		str += '<input type="button" style="background-color: ' + coloroptions[i] + '"' + (typeof setEditorTip == 'function' ? ' onmouseover="setEditorTip(\'' + colortexts[coloroptions[i]] + '\')" onmouseout="setEditorTip(\'\')"' : '') + ' onclick="'
...
/*vot*/		+ '" title="' + colortexts[coloroptions[i]] + '" />' + (i < 39 && (i + 1) % 8 == 0 ? '<br />' : '');
		}
		menu.innerHTML = str;
		$('append_parent').appendChild(menu);
	}
...
function searchBlur(obj) {
	if(obj.value == '' ) {
/*vot*/	obj.value = lng['enter_search_string'];
	}
}
...
/*vot*/		secST['qaa_' + idhash] = setTimeout(function() {$('secqaa_' + idhash).innerHTML = '<span style="cursor:pointer" class="xi2" onclick="updatesecqaa(\''+idhash+'\')">'+lng['refresh_q&a']+'</span>';}, 180000);
		});
	}
}
...
/*vot*/			secST['code_' + idhash] = setTimeout(function() {$('seccode_' + idhash).innerHTML = '<span style="cursor:pointer" class="xi2" onclick="updateseccode(\''+idhash+'\')">'+lng['refresh_code']+'</span>';}, 180000);
			});
		}
	} else {
...
/*vot*/	showDialog(str, 'notice', null, null, 0, null, lng['logging_wait']);
	}
}
