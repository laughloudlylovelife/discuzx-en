...
while ($row = $db_source->fetch_array($query)) {
	...
	!empty($row['contact']) && $row['message'] = $row['message']. lang('update','contacts') .$row['contact'];
...
if($nextid) {
	showmessage(lang('update','continue_convert_table').$table_source." applyid > $nextid", "index.php?a=$action&source=$source&prg=$curprg&start=$nextid");
}
