...
define('PMMSGTONOTFRIEND', -3);//note  group 发短消息，但是目标并非发信者好友
define('PMSENDREGDAYS', -4);//note 发短消息最少 registration 天数
...
class pmcontrol extends base {
	...
	function onsendpm() {
		...
		if($fromuid) {
			...
			$this->load('friend');
			if(count($msgto) > 1 && !($is_friend = $_ENV['friend']->is_friend($fromuid, $msgto, 3))) {//note 如果有 group 发但是目标并非发信人的好友则不允许发送
