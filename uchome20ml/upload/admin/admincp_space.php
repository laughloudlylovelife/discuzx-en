...
if(submitcheck('usergroupsubmit')) {
	...
	if($managespaceinfo) {
		...
		//Reward points for mail activation
		if($emailcheck && $email) {
	...
	if($managename) {
		...
		// Real-name authentication reward points 
	...
	if($_POST['uids'] && is_array($_POST['uids']) && $_POST['optype']) {
		...
		switch ($_POST['optype']) {
			...
			case '2':
				if($managename) {
					//Cancel real name verifyed status
	...
	if($touids) {
		...
		if($replaces) {
			...
			//increase the number of Greetings
			if($uids) {
