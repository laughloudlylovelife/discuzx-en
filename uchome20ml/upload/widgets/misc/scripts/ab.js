...
//  choosePage: '/uchome/widgets/ab_chooser/provider',
  choosePage: '/uchome2/widgets/misc/provider.php',

  //add at 081225
  // E-mail or file upload page
...
{
	var list = new Array();
	list.push({name:l_name_sohu,   id:'Sohu',   logo: site_url+'widgets/misc/images/ab/logo_sohu.gif'});
	list.push({name:l_name_sina,   id:'Sina',   logo: site_url+'widgets/misc/images/ab/logo_sina.gif'});
	list.push({name:l_name_163,    id:'163',    logo: site_url+'widgets/misc/images/ab/logo_163.gif'});
	list.push({name:l_name_yahoo,  id:'Yahoo',  logo: site_url+'widgets/misc/images/ab/logo_yahoo.gif'});
	list.push({name:l_name_gmail,  id:'Gmail',  logo: site_url+'widgets/misc/images/ab/logo_gmail.gif'});
	list.push({name:l_name_msn,    id:'IMMSN',  logo: site_url+'widgets/misc/images/ab/logo_immsn.gif'});
	list.push({name:l_name_qq,     id:'Qzone',  logo: site_url+'widgets/misc/images/ab/logo_imqq.gif'});
//	list.push({name:l_name_icq,    id:'ICQ',    logo: site_url+'widgets/misc/images/ab/logo_icq.gif'});
//	list.push({name:l_name_jabber, id:'Jabber', logo: site_url+'widgets/misc/images/ab/logo_jabber.gif'});
//	list.push({name:l_name_skype,  id:'Skype',  logo: site_url+'widgets/misc/images/ab/logo_skype.gif'});
...
MYABC.showChooser = function(textAreaID, callbackPage, defaultService, isNameInData, useWinOpen, customStyleBaseName)
{
  ...
  var defSer = (defaultService||''); // modified by zgq
  ...
  if (!taObj)
  {
    // Please specify the Contact Selector text field!
    alert(l_select_domain);
    return;
  }
...
MYABC.onSubmit2 = function(form)
{
  ...
  if (actionType == 'cta')
  {
		...
		if(!hasUpload)
    {
      ...
      alert(l_select_file_for_upload);
			return false;
		}
    ...
    if (dotIdx && dotIdx<uploadValue.length)
    {
      ...
      if (!isValid)
      {
        // Select the valid file format!
        alert(l_select_file_format);
        return false;
      }
    }else
    {
...
MYABC.doOnLoad4 = function()
{
  ...
  window.setTimeout(function(){
    ...
    ajax.doAjax(ajaxPage, 'text', function(data){
      ...
      // Fill data is returned to the div in ajax
      // Folder title
      MYABC$('ab_data_folder').innerHTML = jData.folder;

      // Number of items
      MYABC$('ab_data_count').innerHTML = '(' + jData.list.length + ' '+l_projects+')'; // projects
...
MYABC.util.showPopup = function(url, customStyleBaseName)
{
	...
	$myabc_popup_div.id = 'myabc_popup_div_id';
  	...
  	var content = '<div class="' + sname + '_popup_close"><a href="#" onclick="MYABC.util.closeMYABCPopupWin();return false;">['+l_close;+']</a></div>'; //Close
...
MYABC.util.createListItem = function(parentDom, item, idNum, even)
{
  ...
  if ( !MYABC.util.emailHasExisted(item.addr) ) {
    ...
    cols[2].innerHTML = '<span id="my_list_name_' + idNum + '">' + item.name
                  + ' <span class="added">('+l_added+')</span></span>'; // have been added
    cols[3].innerHTML = '<span id="my_list_addr_' + idNum + '">' + item.addr + '</span>';
  }
...
// Parameters: URL, return type ('text' | 'xml' | 'json'), callback functions, error handling function, POST parameters
