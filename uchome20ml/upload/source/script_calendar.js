...
function loadcalendar() {
	...
	s += '<tr align="center" id="calendar_week"><td><a href="###" onclick="refreshcalendar(yy, mm-1)" title="'+l_prev_month+'">&laquo;</a></td><td colspan="5" style="text-align: center"><a href="###" onclick="showdiv(\'year\');doane(event)" class="dropmenu" title="'+l_click_year+'" id="year"></a>&nbsp; - &nbsp;<a id="month" class="dropmenu" title="'+l_click_month+'" href="###" onclick="showdiv(\'month\');doane(event)"></a></td><td><A href="###" onclick="refreshcalendar(yy, mm+1)" title="'+l_next_month+'">&raquo;</A></td></tr>';
	s += '<tr id="calendar_header"><td>'+l_wday0+'</td><td>'+l_wday1+'</td><td>'+l_wday2+'</td><td>'+l_wday3+'</td><td>'+l_wday4+'</td><td>'+l_wday5+'</td><td>'+l_wday6+'</td></tr>';
	for(var i = 0; i < 6; i++) {
		s += '<tr>';
		for(var j = 1; j <= 7; j++)
	...
	s += '<tr id="hourminute"><td colspan="7" align="center"><input type="text" size="2" value="" id="hour" class="txt" onKeyUp=\'this.value=this.value > 23 ? 23 : zerofill(this.value);controlid.value=controlid.value.replace(/\\d+(\:\\d+)/ig, this.value+"$1")\'> '+l_hours+' <input type="text" size="2" value="" id="minute" class="txt" onKeyUp=\'this.value=this.value > 59 ? 59 : zerofill(this.value);controlid.value=controlid.value.replace(/(\\d+\:)\\d+/ig, "$1"+this.value)\'> '+l_mins+'</td></tr>';
	s += '</table></div></div>';
	...
	for(var k = 1; k <= 12; k++) {
		s += '<a href="###" onclick="refreshcalendar(yy, ' + (k - 1) + ');$(\'calendar_month\').style.display=\'none\'"><span' + (today.getMonth()+1 == k ? ' class="calendar_today"' : '') + ' id="calendar_month_' + k + '">' + k + ( k < 10 ? '&nbsp;' : '') + ' '+l_month+'</span></a><br />';
	}
	s += '</div>';
	if(is_ie && is_ie < 7) {
