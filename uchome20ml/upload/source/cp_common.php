...
if($op == 'logout') {
	...
	//Determine whether report was ignored
...
//-------------------------------------------------
	...
	$url = $_SGLOBAL['refer'] ? $_SGLOBAL['refer'] : './'; //vot

	$dir = empty($_GET['name'])?'':str_replace('.','', trim($_GET['name']));
	if($dir && file_exists(S_ROOT.'./template/'.$dir.'/style.css')) {
		ssetcookie('mytemplate', $dir, 3600*24*365);//Store Template
	}
//vot	showmessage('do_success', 'space.php?do=feed', 0);
	showmessage('do_success', $url, 0);

//================================================
//vot:  Change USER LANGUAGE
} elseif($op == 'changelng') {

//$_SGLOBAL['refer']
	$url = $_SGLOBAL['refer'] ? $_SGLOBAL['refer'] : './';

	$dir = empty($_GET['name'])?'':str_replace('.','', trim($_GET['name']));

	if(is_dir(S_ROOT.'./language/'.$dir)) {
		ssetcookie('mylanguage', $dir, 3600*24*365);//Store My Language
	}
	showmessage('do_success', $url, 0);
//================================================
}
