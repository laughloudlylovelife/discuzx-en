...
function ckprivacy($type, $feedmode=0) {
	...
	$_SGLOBAL[$var] = $result;//当前页面 cache 
	return $result;
}
	...
	if(!isset($_SGLOBAL[$var])) {
		if(empty($_SGLOBAL['supe_uid'])) {
			...
			if(substr($permtype, 0, 6) == 'manage' && empty($_SGLOBAL[$var])) {
				$_SGLOBAL[$var] = $_SGLOBAL['usergroup'][$gid]['manageconfig'];// permissions 覆盖
				if(empty($_SGLOBAL[$var])) {
...
function getmtag($id) {
	...
	//成员级别
	$mtag['ismember'] = 0;
	$mtag['grade'] = -9;//-9 非成员 -2 申请 -1 禁言 0 ordinary 1 明星 8 副 group 主 9  group 主
