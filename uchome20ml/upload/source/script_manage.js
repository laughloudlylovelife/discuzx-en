...
function myfriend_post(id, result) {
	if(result) {
		var ids = explode('_', id);
		var uid = ids[1];
		$('friend_'+uid).innerHTML = '<p>'+l_friends_now+': <a href="space.php?uid='+uid+'#comment" target="_blank">'+l_send_msg+'</a> , '+l_or+' <a href="cp.php?ac=poke&op=send&uid='+uid+'" id="a_poke_'+uid+'" onclick="ajaxmenu(event, this.id, 1)">'+l_send_greeting+'</a></p>';
	}
}
//删除好友请求
...
function feed_more_close(feedid) {
	...
	$(opid).innerHTML = '&raquo; '+l_more_dynamic;
	$(opid).onclick = function() {
