...
if($eventid){// 显示 event 内容

	if($view=="me"){//排除由space.php自动 add 的$_GET[view]=me
		$view = "all";
	}
	...
	if(! $event){
		showmessage("event_does_not_exist"); //  event 不存在或者 allready 被 delete 
	}
	if($event['grade'] == 0 && $event['uid'] != $_SGLOBAL['supe_uid'] && !checkperm('manageevent')){
		showmessage('event_under_verify');//  event 正在审核中
	}
	...
	$allowmanage = false; //  event 管理 permissions 
	if($value['status'] >= 3 || checkperm('manageevent')){
		$allowmanage = true;
	}

	// 私密 event ，仅 allready 参加 event 的人和有管理 permissions 的人或有 invite 的人可见
	if($event['public'] == 0 && $_SGLOBAL['supe_userevent']['status'] < 2 && !$allowmanage){
		...
		$value = $_SGLOBAL['db']->fetch_array($query);
		if(empty($value)){
			showmessage("event_not_public"); // 这是一个私密 event ，需要通过 invite 才能查看
		}
	}
	...
	if($view == "member"){
		...
		if($status >= 2){
			$statussql = " AND ue.status >= 2";// 包含 groups 织者
		...
		if ($picid) {
			...
			while($value = $_SGLOBAL['db']->fetch_array($query)){
				if(!$value['filepath']){// photo  allready 经被 delete 
			...
			if($piccount != $event['picnum']) {// update 数目
		...
		//  event  groups 织者
		$relateduids = array();//查找参加此 event 的成员也参加的 event 用
		$admins = array();
	...
	if($needquery) {// 好友的 event allready 特别处理
