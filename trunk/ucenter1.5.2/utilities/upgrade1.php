...
?>
	...
	<p>Upgrade from UCenter 1.0beta to UCenter 1.0 full</p>
	<p>Before running the upgrade, make sure to upload UCenter 1.0 all files and directories</p>
	<p>Strongly recommended that you back up the database before upgrading information</p>
	<p><a href="<?php echo $PHP_SELF;?>?action=updatepw">If you have confirmed the completion of the above steps, please click here to upgrade</a></p>
...
<?php
	...
	//note Deal with config.inc.php

	define('UC_CONFIG', UC_ROOT.'./data/config.inc.php');
	if(!is_writable(UC_CONFIG)) {
		showmessage('Profile ./data/config.inc.php Can not write. (for *nix system, set file permissions to 777, Windows systems set II account is writable).');
	}
	if(empty($_POST['password'])) {
		showheader();
		echo '<br /><br /><h3>Founder of the upgrade process need to reset your password, account for the system built-in, please remember the password.</h3>';
		...
		if($_POST['password'] != $_POST['password2']) {
			showmessage('The two passwords are not equal, please retry.');
		}
	...
	echo "The upgrade is complete. Please delete the upgrade1.php";
	showfooter();
...
<html>
<head>
<title>UCenter upgrade from $version_old to $version_new</title>
...
UCenter Upgrade from $version_old to $version_new)</td>
</tr>
<tr>
<td>
...
<tr>
<td align="center">
<b>The program can upgrade only from $version_old to $version_new , Before running, make sure all the files have been uploaded, and make data backup<br />
If you have any questions about the Upgrade, please visit the Technical Support site <a href="http://www.discuz.net" target="_blank">http://www.discuz.net</a></b>
</td>
</tr>
<tr>
<td>
...
<br /><br />
&gt;&gt;<a href="$url">Browser will automatically redirected, without human intervention. Unless a long time when your browser does not support frames, please click here</a>
<br /><br />
EOT;
	showfooter();
}
