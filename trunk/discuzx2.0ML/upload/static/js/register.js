...
function addFormEvent(formid, focus){
	...
	formNode[stmp[1]].onblur = function () {
		if(formNode[stmp[1]].value == '') {
			errormessage(formNode[stmp[1]].id, lng['password_fill']);
		}else{
...
/*vot*/			errormessage(formNode[stmp[2]].id, lng['password_again']);
	...
	formNode[stmp[3]].onblur = function () {
		if(formNode[stmp[3]].value == '') {
			errormessage(formNode[stmp[3]].id, lng['email_fill']);
		}
...
var emailMenuST = null, emailMenui = 0; //vot emaildomains moved to lang_js.js
