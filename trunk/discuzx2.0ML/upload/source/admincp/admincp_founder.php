...
if($operation == 'perm') {
	...
	if($do == 'group') {
		...
		if(!submitcheck('submit')) {
/*vot*/			$query = DB::query("SELECT cpg.*,cpp.perm FROM ".DB::table('common_admincp_group')." cpg LEFT JOIN ".DB::table('common_admincp_perm')." cpp ON (cpg.cpgroupid=cpp.cpgroupid AND cpp.perm='$gset[1]') ORDER BY cpg.cpgroupid");
			$groups = array();
			while($group = DB::fetch($query)) {
