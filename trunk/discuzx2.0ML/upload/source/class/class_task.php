...
class task {
	...
	function tasklist($item) {
		...
		$updated = FALSE;
		$num = 0;
/*vot*/		$query = DB::query("SELECT t.*, mt.csc, mt.dateline FROM ".DB::table('common_task')." t
...
/*vot*/		$this->task = DB::fetch_first("SELECT t.*, mt.status, mt.csc, mt.dateline, mt.dateline AS applytime FROM ".DB::table('common_task')." t LEFT JOIN ".DB::table('common_mytask')." mt ON (mt.uid='$_G[uid]' AND mt.taskid=t.taskid) WHERE t.taskid='$id' AND t.available='2'");
		if(!$this->task) {
...
/*vot*/		} elseif(!($this->task = DB::fetch_first("SELECT t.taskid, mt.status FROM ".DB::table('common_task')." t LEFT JOIN ".DB::table('common_mytask')." mt ON (mt.taskid=t.taskid AND mt.uid='$_G[uid]') WHERE t.taskid='$id' AND t.available='2'"))) {
			showmessage('task_nonexistence');
		} elseif($this->task['status'] != '0') {
			showmessage('task_not_underway');
		}
