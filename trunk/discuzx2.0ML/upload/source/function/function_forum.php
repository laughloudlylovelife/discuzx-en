...
/*vot*/	if(!empty($_G['gp_archiver'])) {//Archiver X1.5 compatible
		if($fid) {
...
/*vot*/			$accessadd2 = "LEFT JOIN ".DB::table('forum_access')." a ON (a.uid='$_G[uid]' AND a.fid=f.fid)";
		}

		if($adminid == 3) {
			$modadd1 = ', m.uid AS ismoderator';
/*vot*/			$modadd2 = "LEFT JOIN ".DB::table('forum_moderator')." m ON (m.uid='$_G[uid]' AND m.fid=f.fid)";
		}
	}
