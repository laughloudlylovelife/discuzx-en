...
function renewContent() {
	if(window.confirm(l_restore_save)) {
...
function format(type, para){
	...
	if(!gIsIE){
		switch(type){
			case "Cut":
				sAlert = l_bro_not_cut;
				break;
			case "Copy":
				sAlert = l_bro_not_copy;
				break;
			case "Paste":
				sAlert = l_bro_not_paste;
				break;
		}
	}
	if(sAlert != ""){
...
function changeEditType(flag, ev){
	...
	try{
		...
		if(flag){
			...
			ev = ev || event;
			if(ev){
				if(window.confirm(l_plain_lose_format)){
