...
//数据流保存，所有数据均为存放 album 的所以写入的数据一定只能是 image 
...
function feed_add($icon, $title_template='', $title_data=array(), $body_template='', $body_data=array(), $body_general='', $images=array(), $image_links=array(), $target_ids='', $friend='', $appid='', $returnid=0) {
	...
	$feedarr['title_data'] = serialize(sstripslashes($title_data));//数 groups 转化
	$feedarr['body_data'] = serialize(sstripslashes($body_data));//数 groups 转化
	...
	if($uid && $uid != $m_uid) {
		...
		if($friendstatus < 1) {
			...
			if($inviteid) {
				...
				$setarr['uid'] = $m_uid;
				inserttable('invite', $setarr, 0, true);//插入 invite 记录
			}
		}
	}
}
...
function ckvideophoto($type, $tospace=array(), $return=0) {
	...
	} elseif ($tospace['privacy']['view']['video'.$type] == 2) {// user 禁止
		$result = false;
	}
	if($return) {
		return $result;
