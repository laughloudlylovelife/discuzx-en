...
if($id) {
	...
	if($blog['tag'] && empty($blog['related'])) {
		...
		updatetable('blogfield', array('related'=>addslashes(serialize(sstripslashes($blog['related']))), 'relatedtime'=>$_SGLOBAL['timestamp']), array('blogid'=>$blog['blogid']));// update 
	} else {
	...
	if($count) {
		...
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			realname_set($value['authorid'], $value['author']);// real name 
			$list[] = $value;
		}
	}
	...
	if(!$space['self'] && $_SCOOKIE['view_blogid'] != $blog['blogid']) {
		...
		inserttable('log', array('id'=>$space['uid'], 'idtype'=>'uid'));//延迟 update 
