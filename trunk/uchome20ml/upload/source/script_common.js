...
function addSort(obj) {
	if (obj.value == 'addoption') {
 	...
 	newOptDiv.innerHTML = '<h1>'+l_add+'</h1><a href="javascript:;" onclick="addOption(\'newsort\', \''+obj.id+'\')" class="float_del">'+l_delete+'</a><div class="popupmenu_inner" style="text-align: center;">'+l_name+': <input type="text" name="newsort" size="10" id="newsort" class="t_input" /><input type="button" name="addSubmit" value="'+l_create+'" onclick="addOption(\'newsort\', \''+obj.id+'\')" class="button" /></div>';
...
function setCopy(_sTxt){
	if(is_ie) {
		...
		alert (l_url+"&ldquo;"+_sTxt+"&rdquo;\n"+l_copied);
	} else {
		prompt(l_copy_url,_sTxt); 
	}
}
...
function showFlash(host, flashvar, obj, shareid) {
	...
	if(flashAddr[host]) {
		...
		var nodetxt = document.createTextNode(l_stop);
