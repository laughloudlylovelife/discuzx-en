...
function insertAttach(id) {
	...
	if(is_ie && typeof no_insert=='undefined' || insertType==0) {
		localfile += '&nbsp;<a href="javascript:;" title="'+l_click_insert+'" onclick="insertAttachimgTag(' + id + ');return false;">['+l_insert+']</a>';
	}
	localfile += '&nbsp;<span id="showmsg' + id + '"><a href="javascript:;" onclick="delAttach(' + id + ')">['+l_delete+']</a></span>';
	inhtml += '<td>' + localfile +'<br/>';
	inhtml += l_img_descr+':<br/><textarea name="pic_title" cols="40" rows="2"></textarea>';
...
function upload() {
	...
	if(nowUid>0) {
		...
		if(uploadStat==1) {
			...
			upobj.innerHTML = l_upload_failed+' '+uploadStat;
		}
	}
	if($('showmsg'+nid) != null) {
		$('showmsg'+nid).innerHTML = l_wait_upload+' (<a href="javascript:;" onclick="forms[nowUid].submit();">'+l_try_again+'</a>)';
