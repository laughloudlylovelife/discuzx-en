...
$maxcount = 50;//Max Invited Friend
...
//Processing e-mail Invited
if(submitcheck('emailinvite')) {
	set_time_limit(0);//Set the timeout
...
function createmail($mail, $mailvar) {
	...
	smail(0, $mail, cplang($appinfo ? 'app_invite_subject' : 'invite_subject', array($_SN[$space['uid']], $_SCONFIG['sitename'], $appinfo['appname'])), cplang($appinfo ? 'app_invite_message' : 'invite_message', $mailvar));
}
?>