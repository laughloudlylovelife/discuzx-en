...
define('UC_NOTE_REPEAT', 5);	//note  notice 重复次数
define('UC_NOTE_TIMEOUT', 15);	//note  notice 超时时间(秒)
define('UC_NOTE_GC', 10000);	//note 过期 notice 的回收概率，该值越大，概率越低
...
class notemodel {
	...
	function notemodel(&$base) {
		...
		/** note
		 * 1. 操作的 name 称，如： delete  user ，测试连通， delete 好友，取TAG数据， update 客户端 cache 
	 ...
	 * @param array appids 指定 notice 的 APPID
	...
	function _send() {
		...
		//note 如果内存表记录不存在，那么可能 mysql 被重启，需要再次判断 notice 是否存在
	...
	function sendone($appid, $noteid = 0, $note = '') {
		...
		$returnsucceed = $response != '' && ($response == 1 || is_array(xml_unserialize($response)));//note 当确实返回为1的时候才认为是 notice success 
