...
define('UC_ROOT', substr(__FILE__, 0, -10));		//note  user 中心客户端的根目录 UC_CLIENTROOT
define('UC_DATADIR', UC_ROOT.'./data/');		//note  user 中心的数据 cache 目录
define('UC_DATAURL', UC_API.'/data');			//note  user 中心的数据 URL
...
/**
 *  dfopen  mode 取指定的模块和动作的数据
 *
 * @param string $module	请求的模块
 * @param string $action 	请求的动作
 * @param array $arg		参数（会加密的 mode 传送）
 * @return string
 */
 ...
 * @param string $module	请求的模块
 * @param string $action	请求的动作
 * @param string $arg		参数（会加密的 mode 传送）
...
/**
 * MYSQL  mode 取指定的模块和动作的数据
 *
 * @param string $model		请求的模块
 * @param string $action	请求的动作
 * @param string $args		参数（会加密的 mode 传送）
 * @return mix
 */
 ...
 * @return int
	-1 :  user  name 不合法
	-2 : 包含不允许 registration 的词语
	-3 :  user  name  allready 经存在
	-4 : email 格式有误
	-5 : email 不允许 registration 
	-6 : 该 email  allready 经被 registration 
	>1 : 表示 success ，数值为 UID
*/
 ...
 *  delete  user 
 *
 * @param string/array $uid	 user 的 UID
 * @return int
 	>0 :  success 
 	0 : 失败
 */
 ...
 *  add 保护 user 
 *
 * @param string/array $username 保护 user  name 
...
/**
 * 得到受保护的 user  name  list 
 *
 * @param empty
 * @return
 * 	受到保护的 user  name  list 
 *  	array()
 */
 ...
 *  user 合并最后的处理
 *
 * @param string $oldusername	老 user  name 
 * @param string $newusername	新 user  name 
...
/**
 * 获取指定应用的指定 user  points 值
 * @param int $appid	应用Id
 * @param int $uid	 user Id
 * @param int $credit	 points 编号
 */
 ...
 * 发送短消息
 *
 * @param int $fromuid		发件人uid 0 为 system 消息
 ...
 * @param int $replypid		 reply 的消息Id
 ...
 * @param array	$pmids		要 delete 的消息ID数 groups 
 * @return
 * 	>0  success 
 * 	<=0 失败
 */
 ...
 *
 * @param int $uid		 user Id
 * @param array	$uids		要 delete 的消息 user ID数 groups 
 * @return
 * 	>0  success 
 * 	<=0 失败
 */
 ...
 *
 * @param int $uid		 user Id
 * @param array	$uids		要标记 allready 读状态的 user ID数 groups 
 * @param array	$pmids		要标记 allready 读状态的消息ID数 groups 
 * @param int $status		1  allready 读 0 未读
 */
 ...
 * @param string $filter	过滤 mode newpm=未读消息，systempm= system 消息，announcepm=公共消息
 				$folder		$filter
 				--------------------------
 				newbox
 				inbox		newpm
 ...
 * @param int $uid		 user Id
 * @param int $pmid		消息Id
 * @param int $touid		消息对方 user Id
 * @param int $daterange	日期范围 1=今天,2=昨天,3=前天,4=上周,5=更早
 * @return array() 短消息内容数 groups 
 */
 ...
 * @param int $type		0 = 获取指定单条消息
 				1 = 获取指定 user 发的最后单条消息
 				2 = 获取指定 user 收的最后单条消息
 * @return array() 短消息内容数 groups 
 */
 ...
 *
 * @param int $uid		 user ID
 * @param int $from		原 points 
 * @param int $to		目标 points 
 * @param int $toappid		目标应用ID
 * @param int $amount		 points 数额
 * @return
 *  	1  :  success 
 *	0  : 失败
 */
 ...
 * 返回指定的相关TAG数据
 *
 * @param string $tagname	TAG name 称
 * @param int $totalnum		返回数据的条目数
 * @return array() 序列化过的数 groups ，数 groups 内容为当前或其他应用的相关TAG数据
 */
 ...
 * 邮件队列
 *
 * @param	string	$uids		user name id，多个用逗号(,)隔开
 ...
 * @return	integer
 *		=0 : 失败
 *		>0 :  success ，返回插入记录的id，如果是多条则返回最后一条记录的id，若level等于0，则返回1
 */
