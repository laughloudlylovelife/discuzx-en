...
if(empty($_GET['op'])) {
	...
	$_SGLOBAL['db']->query('SET SQL_QUOTE_SHOW_CREATE=0', 'SILENT');//Error-free execution off the table and I create a column without quotes
	if(empty($filename) || strlen($filename) > 40){	//Determine the length of the file name
	...
	if($type == 'uchomes') {
		...
		if(isset($_POST['setup'])) {//POST submit backup
	...
	if($method == 'multivol') {//Sub-volume backup
		$sqldump = '';
		$sizelimit = intval(getval('sizelimit'));
		$tableid = intval(getval('tableid'));//Table ID
		$startfrom = intval(getval('startfrom'));//Starting position
	...
	if($_GET['do'] == 'zip') {
		...
		if($identify[3] == 'multivol') {
			...
			foreach($sqlquery as $sql) {
				...
				if(!empty($sql)) {
					$_SGLOBAL['db']->query($sql, 'SILENT');//Ignore error
...
function sqldumptable($table, $startfrom = 0, $currsize = 0) {
	...
	if($_GET['extendins'] == 0){
		while($currsize + strlen($tabledump) + 500 < $filesize && $numrows == $offset){
			...
			$numfields = $_SGLOBAL['db']->num_fields($query);	//Obtain the number of columns

			if($numrows = $_SGLOBAL['db']->num_rows($query)) {
				while($row = $_SGLOBAL['db']->fetch_row($query)) {	//Enumerate the values ??obtained in the form line
					$dumpsql = $comma = '';
