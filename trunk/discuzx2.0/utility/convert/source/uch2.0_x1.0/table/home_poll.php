...
if(!$fid) {
	...
	if(!empty($value)) {
		...
		$forumfield = array(
			'fid' => $fid,
			'description' => lang('uchome_polls_convert')
		);
...
if($nextid) {
	showmessage(lang('continue_convert_table').$table_source." pid > $nextid", "index.php?a=$action&source=$source&prg=$curprg&start=$nextid&fid=$fid");
}
