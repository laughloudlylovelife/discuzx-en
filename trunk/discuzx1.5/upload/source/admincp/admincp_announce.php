...
if(empty($operation)) {
	...
	if(!submitcheck('addsubmit')) {
		...
		$newstarttime = dgmdate(TIMESTAMP, 'Y-m-d');
		$newendtime = dgmdate(TIMESTAMP + 86400* 7, 'Y-m-d');
	...
	if(!submitcheck('editsubmit')) {
		...
		$announce['starttime'] = $announce['starttime'] ? dgmdate($announce['starttime'], 'Y-m-d') : "";
		$announce['endtime'] = $announce['endtime'] ? dgmdate($announce['endtime'], 'Y-m-d') : "";
