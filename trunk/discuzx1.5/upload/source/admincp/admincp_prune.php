...
if(!submitcheck('prunesubmit')) {
	...
	$starttime = !preg_match("/^(0|\d{4}\-\d{1,2}\-\d{1,2})$/", $_G['gp_starttime']) ? dgmdate(TIMESTAMP - 86400 * 7, 'Y-m-d') : $_G['gp_starttime'];
	$endtime = $_G['adminid'] == 3 || !preg_match("/^(0|\d{4}\-\d{1,2}\-\d{1,2})$/", $_G['gp_endtime']) ? dgmdate(TIMESTAMP, 'Y-m-d') : $_G['gp_endtime'];
...
EOT;
	...
	empty($_G['gp_starttime']) && $_G['gp_starttime'] = dgmdate(TIMESTAMP - 86400 * 7, 'Y-m-d');
...
if(submitcheck('searchsubmit', 1)) {
	...
	if($_G['adminid'] == 1 && $_G['gp_endtime'] != dgmdate(TIMESTAMP, 'Y-m-d')) {
