...
if(empty($eventfid)) {
	$board_name = lang('update','uchome_data');
	$forum_name = lang('update','uchome_events');
	...
	if(!empty($value)) {
		...
		$forumfield = array(
			'fid' => $eventfid,
			'description' => lang('update','uchome_events_convert')
		);
...
while ($event = $db_source->fetch_array($event_query)) {
	...
	$activityarr = array(
		...
		'applynumber' => $event['membernum'] - 1,// Home events of members, including founder
...
if($nextid) {
	showmessage(lang('update','continue_convert_table').$table_source." eventid> $nextid", "index.php?a=$action&source=$source&prg=$curprg&start=$nextid");
}
