...
while ($space = $db_source->fetch_array($query)) {
	...
	if(empty($home['extcredits']['credit']) || empty($home['extcredits']['experience'])) {
		showmessage(lang('update','space_home_error'));
	}
...
if($nextid) {
	showmessage(lang('continue_convert_table'). " {$oldpre}space uid> $nextid", "index.php?a=$action&source=$source&prg=$curprg&start=$nextid");
}
