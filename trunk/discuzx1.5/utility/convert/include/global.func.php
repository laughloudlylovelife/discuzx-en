...
function timetounix($time) {
	...
	// Chinese Months
	$time = str_replace(array(' 一月 ',' 二月 ',' 三月 ',' 四月 ',' 五月 ',' 六月 ',' 七月 ',' 八月 ',' 九月 ',' 十月 ',' 十一月 ',' 十二月 ', ' 上午 '), array('-01-','-02-','-03-','-04-','-05-','-06-','-07-','-08-','-09-','-10-','-11-','-12-', ' '), $time);
	// English Months
	$time = str_ireplace(array(lang('update','jan'),lang('update','feb'),lang('update','mar'),lang('update','apr'),lang('update','may'),lang('update','jun'),lang('update','jul'),lang('update','sep'),lang('update','oct'),lang('update','nov'),lang('update','dec'),lang('update','am')), array('-01-','-02-','-03-','-04-','-05-','-06-','-07-','-08-','-09-','-10-','-11-','-12-', ' '), $time);

	$pm = 0; //vot

	if(strrchr($time, '下午') !== false) { // Chinese "PM"
		$pm = 43200;
                $time = str_replace(' 下午 ', ' ', $time);
	} elseif(stristr($time, lang('update','pm')) !== false) { // English "PM"
		$pm = 43200;
                $time = str_ireplace(lang('update','pm'), ' ', $time);
	}
...
function showmessage($message, $url_forward = '', $vars = array(), $timeout = 1) {
	showheader();
	$message = lang('update',$message, $vars);
	$messageadd = '';
	$timeout = intval($timeout);
	if($url_forward) {
		$messageadd .= "<script>setTimeout(\"redirect('$url_forward');\", $timeout);</script>";
		$messageadd .= "&gt;&gt;<a href=\"index.php\"><strong>".lang('update','message_stop')."</strong></a>";
	} elseif(strpos($message, lang('update','return'))) {
		$messageadd .= "<a href=\"javascript:history.go(-1);\" class=\"mediumtxt\">".lang('update','message_return').'</a><br><br>';
	}

	$prompt = lang('update','prompt');
	echo <<<EOT
...
EOT;
	...
	echo "
<h1>".lang('update','dzx_update_wizard')." $titleadd</h1>
<div class='content'>
<table id='menu'>
	<tr>
	<td $class[source]>".lang('update','step1')." </td>
	<td $class[config]>".lang('update','step2')." </td>
	<td $class[select]>".lang('update','step3')." </td>
	<td $class[convert]>".lang('update','step4')." </td>
	<td $class[finish]>".lang('update','step5')." </td>
	</tr>
</table>
";
...
function lang($file, $name, $vars = array()) { //vot
	static $lang;
	if($lang === null) {
		@include DISCUZ_ROOT.'./language/lang_update.php';
		if(empty($lang)) {
			$lang = array();
		}
	}
	$ret = isset($lang[$name]) ? $lang[$name] : $name;
	if(!empty($vars)) {
...
function show_form_footer($submitname = '', $submitvalue = 'submit') {
	if($submitname != '') {
		$submitvalue = lang('update',$submitvalue);
		echo <<<EOT
<table class="button">
...
function show_config_input($type, $config, $error = array()) {
	...
	foreach ($config as $key => $value) {
		...
		array('width="150"', lang('update','config_'.$key)),
