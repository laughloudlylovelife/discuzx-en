...
if(!file_exists(UC_ROOT.'./data/config.inc.php')) {
	echo 'Please copy this program to the UCenter directory and then execute this program.';
	exit;
}
...
if($step === '') {
	...
	if(!$db->num_rows($query)) {
		...
		echo 'Converting the users short messages: '.$percent.'% ...';
	...
	echo '<hr size=1>';
	echo 'Old UCenter Short messages entries: '.$totalcount.'<br />Converted UCenter short message number: '.$pmcount.'<br /><br /><a href="pmconvert.php?step=4">Normal, delete the old data backup</a> <a href="pmconvert.php?step=5">Not normal, recheck the conditions for conversion and restore the data</a>';
	...
	echo '<hr size=1>';
	echo '<br /><br /><a href="pmconvert.php">Back to the start page</a>';
