...
if(empty($eventfid)) {
	$board_name = lang('uchome_data');
	$forum_name = lang('uchome_events');
	...
	if(!empty($value)) {
		...
		$forumfield = array(
			'fid' => $eventfid,
			'description' => lang('uchome_events_convert')
		);
...
if($nextid) {
	showmessage(lang('continue_convert_table').$table_source." eventid> $nextid", "index.php?a=$action&source=$source&prg=$curprg&start=$nextid");
}
