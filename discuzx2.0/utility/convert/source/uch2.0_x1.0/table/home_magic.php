...
if($op=='magic') {
	...
	showmessage(lang('continue_convert_table').$table_source. lang('user_magic'), "index.php?a=$action&source=$source&prg=$curprg&op=usermagic&start=0");
	...
	if($done == false) {
		showmessage(lang('continue_convert_table').$table_source. lang('user_magic')."-> $nextid", "index.php?a=$action&source=$source&prg=$curprg&op=usermagic&start=$nextid");
	} else {
		showmessage(lang('continue_convert_table').$table_source. lang('magic_records')."->0", "index.php?a=$action&source=$source&prg=$curprg&op=magicinlog&start=0");
	}
	...
	if($done == false) {
		showmessage(lang('continue_convert_table').$table_source. lang('magic_records')."-> $nextid", "index.php?a=$action&source=$source&prg=$curprg&op=magicinlog&start=$nextid");
	} else {
		showmessage(lang('continue_convert_table').$table_source. lang('magic_use_records')."->0", "index.php?a=$action&source=$source&prg=$curprg&op=magicuselog&start=0");
	}
	...
	if($done == false) {
		showmessage(lang('continue_convert_table').$table_source. lang('magic_use_records')."-> $nextid", "index.php?a=$action&source=$source&prg=$curprg&op=magicuselog&start=$nextid");
	}
}
